...
logging.basicConfig(filename='pythonlogs\DailySetup6E' + str(datetime.date.today()) + '.txt', level=logging.DEBUG, format='%(asctime)s - %(levelname)s - %(message)s')
logger = logging.getLogger()
logger.setLevel(logging.DEBUG)

formatter = logging.Formatter('%(asctime)s - %(levelname)s - %(message)s')

fh = logging.FileHandler('DailySetup6E' + str(datetime.date.today()) + '.txt')
...
#            order.m_transmit = transmit
       	...
       	# ENTRY   A simple stop order
            ...
            if action == 'BUY':
                ...
                lmtPrice = truncate((float(tHigh[0]) + 0.001),4)
                logger.debug('Buy Stop Limit"Limit" Price or better to buy %s', lmtPrice)
                order.m_lmtPrice  = lmtPrice
                
                stopPrice = truncate((float(tHigh[0]) + 0.001),4)
...
while Flag == 0:            
    ...
    qqq = Contract()  
    qqq.m_symbol = 'M6E'
    qqq.m_localSymbol = 'M6E'+ datalink.monthcode + '8' 
    #qqq.ConId = 279555803
    qqq.m_secType = 'FUT'  
...
if (CurrentPositions[0] < 1000000):  ##Removes Positions limits for BB Strat
    ...
    Price = float(truncate(float(yHigh[0]),2)) + 1.00  #Entry Price
...
#LONGS    
    ...
    if (float(MasterFill[0]) == 0 and float(EntryPend[0]) == 0 and float(tClose[0]) > float(MovAvg[0]) and c < (float(tHigh[0])+ 0.001)): #float(tClose[0]) > float(MovAvg[0]) and float(yClose[0]) < float(yOpen[0]) and float(tClose[0]) > float(tOpen[0]) and float(yLow[0]) < float(yLowerBand[0])):
        ...
        Currency = CCY1+CCY2
        TriggerID = OID
        Price = round(float(truncate(float(tHigh[0]),4)) + 0.001,4)
...
#        logger.debug('idOrder is %s ', idOrder)
 #REMOVE PROFIT TARGET IN STRAT AND MOVE TO POSMGTM AS PART OF OCA WITH STOP LOSS       
        ...
        Price = (float(tHigh[0]) + 0.01)  # Keep this to ensure Orders shows proper Profit Target
        #cur.execute("""Insert Into Orders (idOrder, Date, Time, Currency, TriggerID, Price, Strategy, Bracket, Status) values(%s,%s,%s,%s,%s,%s,%s,%s,%s)""",(idOrder, Date, time, Currency, TriggerID, Price, Strategy, Bracket, Status))
        #cur.execute(query)
...
#Put STOPLOSS Value in hasPosition Table as Placeholder
        StopLossvalue = truncate(min((float(tLow[0]) - 0.007),(float(tHigh[0]) - 0.012)),4)
...
#SHORT TRADES 
    ...
    Currency = CCY1+CCY2
    TriggerID = OID  #Groups Parent and Child orders to single parent order ID
    Price = round(float(truncate(float(tLow[0]),4)) - 0.0010,2)
...
#    logger.debug('yUpperBand is %s', yUpperBand[0])
    ...
    if (float(MasterFill[0]) == 0 and float(EntryPend[0]) == 0 and float(tClose[0]) <= float(MovAvg[0]) and c > (float(tLow[0]) - 0.001)): #float(tClose[0]) > float(MovAvg[0]) and float(yClose[0]) < float(yOpen[0]) and float(tClose[0]) > float(tOpen[0]) and float(yLow[0]) < float(yLowerBand[0])):
...
#        idOrder = OID
        ...
        StopLossvalue = truncate(max((float(tHigh[0]) + 0.0070),(float(tLow[0]) + 0.012)),4) #Either 2 pips above the High or 10 pips from the low, whichever is more
        query = ("UPDATE hasPosition SET BB_StopLossVal = " + str(StopLossvalue) + " WHERE CCY =\'" + CCY1 + CCY2+"\';")
        cur.execute(query)
        logger.debug('hasPosition query is %s',query)
        cnx.commit()
