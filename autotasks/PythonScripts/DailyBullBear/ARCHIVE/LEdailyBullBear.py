...
#            order.m_transmit = transmit
       	...
       	# ENTRY   A simple stop order
            ...
            if action == 'BUY':
                ...
                lmtPrice = truncate(round((float(tHigh[0]) + 0.05),3),3)
                logger.debug('PRICE %s', round((float(tHigh[0]) + 0.05),3))
                logger.debug('Buy Stop Limit"Limit" Price or better to buy %s', lmtPrice)
                order.m_lmtPrice  = lmtPrice
                
                stopPrice = truncate(round((float(tHigh[0]) + 0.05),3),3)
...
#LONGS    
    ...
    if (float(MasterFill[0]) == 0 and float(EntryPend[0]) == 0 and float(tClose[0]) > float(MovAvg[0])): #float(tClose[0]) > float(MovAvg[0]) and float(yClose[0]) < float(yOpen[0]) and float(tClose[0]) > float(tOpen[0]) and float(yLow[0]) < float(yLowerBand[0])):
        ...
        Currency = CCY1+CCY2
        TriggerID = OID
        Price = round(float(truncate(float(tHigh[0]),2)) + 0.050,3)
...
#SHORT TRADES 
    ...
    Currency = CCY1+CCY2
    TriggerID = OID  #Groups Parent and Child orders to single parent order ID
    Price = round(float(truncate(float(tLow[0]),4)) - 0.050,3)
