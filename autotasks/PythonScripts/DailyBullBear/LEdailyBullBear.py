...
logging.basicConfig(filename='pythonlogs\DailySetupLE' + str(datetime.date.today()) + '.txt', level=logging.DEBUG, format='%(asctime)s - %(levelname)s - %(message)s')
logger = logging.getLogger()
logger.setLevel(logging.DEBUG)

formatter = logging.Formatter('%(asctime)s - %(levelname)s - %(message)s')

fh = logging.FileHandler('DailySetupLE' + str(datetime.date.today()) + '.txt')
...
masteraccount = datalink2.DB_Account
...
#            order.m_transmit = transmit
       	...
       	# ENTRY   A simple stop order
            ...
            if action == 'BUY':
                ...
                lmtPrice = truncate(round((float(tHigh[0]) + 0.30),3),3)
                logger.debug('PRICE %s', round((float(tHigh[0]) + 0.30),3))
                logger.debug('Buy Stop Limit"Limit" Price or better to buy %s', lmtPrice)
                order.m_lmtPrice  = lmtPrice
                
                stopPrice = truncate(round((float(tHigh[0]) + 0.30),3),3)
...
def create_contract(symbol, sec_type, exch, prim_exch, curr):
    ...
    contract.m_localSymbol = CCY1+CCY2 + datalink2.monthcode + "8";
...
# '" + str(int(BB_ID)) + "\';"
query = ("SELECT Open from " + CCY1 + CCY2 + " where ID = " + str(BB_ID[0]) + ";")
...
query = ("SELECT High from " + CCY1 + CCY2 + " where ID = " + str(BB_ID[0]) + ";")
cur.execute(query)
for (High) in cur:
    tHigh = High
...
query = ("SELECT Low from " + CCY1 + CCY2 + " where ID = " + str(BB_ID[0]) + ";")
cur.execute(query)
for (Low) in cur:
   tLow = Low
...
query = ("SELECT Close from " + CCY1 + CCY2 + " where ID = " + str(BB_ID[0]) + ";")
cur.execute(query)
for (Close) in cur:
    tClose = Close
...
cur.execute(query)
for (RiskParametersValue) in cur:
    MasterFill = RiskParametersValue
    logger.debug('Master Fill is %s', MasterFill)
...
for (ID) in cur:
    yBB_ID = ID
    logger.debug('ID for Last trading period--1 Day ago-- is %s', yBB_ID)

query = ("SELECT Open from " + CCY1 + CCY2 + " where ID = " + str(yBB_ID[0]) + ";")
logger.debug('query is %s', query)
cur.execute(query)
for (Open) in cur:
    yOpen = Open
...
query = ("SELECT High from " + CCY1 + CCY2 + " where ID = " + str(yBB_ID[0]) + ";")
logger.debug('query is %s', query)
cur.execute(query)
for (High) in cur:
    yHigh = High
...
query = ("SELECT Low from " + CCY1 + CCY2 + " where ID = " + str(yBB_ID[0]) + ";")
logger.debug('query is %s', query)
cur.execute(query)
for (Low) in cur:
    yLow = Low
...
query = ("SELECT Close from " + CCY1 + CCY2 + " where ID = " + str(yBB_ID[0]) + ";")
logger.debug('query is %s', query)
cur.execute(query)
for (Close) in cur:
    yClose = Close
...
while Flag == 0:            
    ...
    time.sleep(1)
    conn.register(reply_handler_price,'HistoricalData')   #By registering "HistoricalData" --the Method name only --we can eliminate all the open order garbage
    ...
    qqq = Contract()  
    qqq.m_symbol = 'LE'
    qqq.m_localSymbol = 'LE'+ datalink2.monthcode + '8' 
    #qqq.ConId = 279555803
    qqq.m_secType = 'FUT'  
    ...
    #conn.reqHistoricalData(1, qqq, '', '1 S', '1 min', 'ASK', 0, 1)
    conn.reqHistoricalData(1, qqq, '', '60 S', '1 min', 'ASK', 0, 1) 
...
if (CurrentPositions[0] < 1000000):  ##Removes Positions limits for BB Strat
    ...
    action   = 'BUY'
    counter_action = 'SELL'
    logger.debug('Checking 2 and half')
    idOrder = int(OID)
    ...
    Price = float(truncate(float(yHigh[0]),2)) + 0.05  #Entry Price
    Strategy = "DailyBullBear_Long"
    Bracket = "Entry"
    Status = "Submitted"
    parentId = OID
    logger.debug('Checking 3')
...
#LONGS    
    # and float(yClose[0]) < float(yOpen[0]) and float(tClose[0]) > float(tOpen[0]) and float(yLow[0]) < float(yLowerBand[0])
    if (float(MasterFill[0]) == 0 and float(EntryPend[0]) == 0 and float(tClose[0]) > float(MovAvg[0]) and c < (float(tHigh[0])+0.3)): #float(tClose[0]) > float(MovAvg[0]) and float(yClose[0]) < float(yOpen[0]) and float(tClose[0]) > float(tOpen[0]) and float(yLow[0]) < float(yLowerBand[0])):
        ...
        Currency = CCY1+CCY2
        TriggerID = OID
        Price = round(float(truncate(float(tHigh[0]),2)) + 0.30,3)
...
#        logger.debug('idOrder is %s ', idOrder)
 #REMOVE PROFIT TARGET IN STRAT AND MOVE TO POSMGTM AS PART OF OCA WITH STOP LOSS       
        ...
        Price = (float(tHigh[0]) + 11)  # Keep this to ensure Orders shows proper Profit Target
        #cur.execute("""Insert Into Orders (idOrder, Date, Time, Currency, TriggerID, Price, Strategy, Bracket, Status) values(%s,%s,%s,%s,%s,%s,%s,%s,%s)""",(idOrder, Date, time, Currency, TriggerID, Price, Strategy, Bracket, Status))
        #cur.execute(query)
...
#Put STOPLOSS Value in hasPosition Table as Placeholder
        StopLossvalue = truncate(min((float(tLow[0]) - 0.25),(float(tHigh[0]) - 1.25)),2)
...
#SHORT TRADES 
    ...
    Currency = CCY1+CCY2
    TriggerID = OID  #Groups Parent and Child orders to single parent order ID
    Price = round(float(truncate(float(tLow[0]),4)) - 0.30,3)
    Strategy = "DailyBullBear_short"
    Bracket = "Entry"
    Status = "Submitted"
    parentId = OID
    #RiskReward = RiskRewardRatio
    ...
    #logger.debug('tOpen is %s', float(tOpen[0]))
    logger.debug('yLow is %s', float(yLow[0]))
    logger.debug('tLow is %s', float(tLow[0]))
    logger.debug('c is %s', c)
#    logger.debug('yUpperBand is %s', yUpperBand[0])
    logger.debug('Current Position is %s', CurrentPositions)
    logger.debug('Checking for SHORT trade entry')
    # and float(yClose[0]) > float(yOpen[0]) and float(tClose[0]) < float(tOpen[0]) and float(yHigh[0]) > float(yUpperBand[0])
    if (float(MasterFill[0]) == 0 and float(EntryPend[0]) == 0 and float(tClose[0]) <= float(MovAvg[0]) and c > (float(tLow[0]) - 0.3)): #float(tClose[0]) > float(MovAvg[0]) and float(yClose[0]) < float(yOpen[0]) and float(tClose[0]) > float(tOpen[0]) and float(yLow[0]) < float(yLowerBand[0])):
...
#Create STP Sell order 5 pips below Low
 ...
 #Update CurrentPosition
        ...
        #cur.execute(query)
        #cnx.commit()
        #logger.debug('Master Pending set to 1')
...
#        idOrder = OID
        ...
        StopLossvalue = truncate(max((float(tHigh[0]) + 0.25),(float(tLow[0]) + 1.25)),2) #Either 2 pips above the High or 10 pips from the low, whichever is more
        query = ("UPDATE hasPosition SET BB_StopLossVal = " + str(StopLossvalue) + " WHERE CCY =\'" + CCY1 + CCY2+"\';")
        cur.execute(query)
        logger.debug('hasPosition query is %s',query)
        cnx.commit()
