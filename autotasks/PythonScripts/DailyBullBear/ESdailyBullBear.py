...
fh = logging.FileHandler('DailySetupES' + str(datetime.date.today()) + '.txt')
...
#LONGS    
    ...
    if (float(MasterFill[0]) == 0 and float(EntryPend[0]) == 0 and float(tClose[0]) > float(MovAvg[0]) and c < (float(tHigh[0])+1.5)): #float(tClose[0]) > float(MovAvg[0]) and float(yClose[0]) < float(yOpen[0]) and float(tClose[0]) > float(tOpen[0]) and float(yLow[0]) < float(yLowerBand[0])):
        ...
        Currency = CCY1+CCY2
        TriggerID = OID
        Price = round(float(truncate(float(tHigh[0]),2)) + 1.50,2)
...
#SHORT TRADES 
    ...
    Currency = CCY1+CCY2
    TriggerID = OID  #Groups Parent and Child orders to single parent order ID
    Price = round(float(truncate(float(tLow[0]),4)) - 1.50,2)
...
#    logger.debug('yUpperBand is %s', yUpperBand[0])
    ...
    if (float(MasterFill[0]) == 0 and float(EntryPend[0]) == 0 and float(tClose[0]) <= float(MovAvg[0]) and c > (float(tLow[0]) - 1.5)): #float(tClose[0]) > float(MovAvg[0]) and float(yClose[0]) < float(yOpen[0]) and float(tClose[0]) > float(tOpen[0]) and float(yLow[0]) < float(yLowerBand[0])):
