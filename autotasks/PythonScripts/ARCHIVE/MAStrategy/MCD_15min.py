...
logging.basicConfig(filename='pythonlogs\MAPricing' + str(datetime.date.today()) + '.txt', level=logging.DEBUG, format='%(asctime)s - %(levelname)s - %(message)s')
logger = logging.getLogger()
logger.setLevel(logging.DEBUG)

formatter = logging.Formatter('%(asctime)s - %(levelname)s - %(message)s')

fh = logging.FileHandler('MAPricing' + str(datetime.date.today()) + '.txt')
...
conn.reqHistoricalData(6, qqq, '', '60 S', '15 mins', 'ASK', 0, 1) 
