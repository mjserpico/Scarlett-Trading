...
logger.debug('Yesterday2 is %s', type(yesterday2))    
logger.debug('Entry Date %s', entry_date)
logger.debug('Entry Date %s', type(entry_date))

logger.debug('CurCount is %s', curCount[0])
logger.debug('CurCount type %s', type(curCount[0]))
...
if (int(curCount[0]) == 0 and yesterday2 == str(entry_date) and int(api_id) == int(Order_profit[0])):
...
logger.debug('Stop Quantity is %s', StopQty)  
 ...
 ###Break even Function       
#if (int(curCount[0] == 1) and float(test4) >= breakeven_price[0]):
