...
def make__short__order(action, qty, limit = None, profit_take=None, training_stop_percent=None, transmit=True, parentId=None):
        ...
        if limit == 2:
            ...
            if action == 'BUY':
                ...
                order.m_lmtPrice  = truncate(float(tLow[0]) - ((float(truncate(float(tHigh[0]),2))- float(truncate(float(tLow[0]),2))) * int(RiskReward[0])),2)
       	...
       	# ENTRY   A simple stop order
            ...
            if action == 'BUY':
                ...
                logger.debug('RiskReward Ratio is %s', RiskRewardRatio)
                order.m_lmtPrice  = truncate(float(tLow[0]) - ((float(truncate(float(tHigh[0]),2))- float(truncate(float(tLow[0]),2))) * int(RiskReward[0])),2)
                stopPrice = truncate(float(tLow[0]) - ((float(truncate(float(tHigh[0]),2))- float(truncate(float(tLow[0]),2))) * int(RiskReward[0])),2)
            ...
            if action == 'SELL':
                ...
                order.m_lmtPrice  = float(truncate(float(tLow[0]),2)) - 0.05
                stopPrice = float(truncate(float(tLow[0]),2)) - 0.05
...
def make_order2(action, qty, limit = None, profit_take=None, training_stop_percent=None, transmit=True, parentId=None):
        ...
        if limit == 2:
            if action == 'SELL':
                print(ProfitPips)
                order.m_lmtPrice  = truncate(float(tHigh[0]) + ((float(truncate(float(tHigh[0]),2))- float(truncate(float(tLow[0]),2))) * int(RiskReward[0])),2)
...
#                print(RiskRewardRatio)
       	...
       	# ENTRY   A simple stop order
            ...
            if action == 'BUY':
                ...
                #order.m_lmtPrice  = limit - int(np.around((limit*profit_take_percent)/100.0, 5)/0.00005)*0.00005
                order.m_lmtPrice  = float(truncate(float(tHigh[0]),2)) + 0.05
                stopPrice = float(truncate(float(tHigh[0]),2)) + 0.05
                order.m_auxPrice = stopPrice;
                order.m_parentId = parentId
                order.m_account = masteraccount
            elif action == 'SELL':
...
if (CurrentPositions < MaxPositions):
    ...
    Price = round(float(truncate(float(tHigh[0]),2)) + 0.05,2)
...
# create a profit take order of some kind
        limit = 2
        Price = round(float(tHigh[0]) + ((float(truncate(float(tHigh[0]),2))- float(truncate(float(tLow[0]),2))) * int(RiskReward[0])),2)
        ...
        StopLossvalue = round(float(truncate(float(tLow[0]),2)) - 0.05,2)
        query = ("UPDATE hasPosition SET StopLossVal = " + str(StopLossvalue) + " WHERE CCY =\'" + CCY1 + CCY2+"\';")
        logger.debug('query is %s', query)
        cur.execute(query)
        cnx.commit()
        
        Breakeven = round(float(truncate(float(tHigh[0]),2)) + 0.05,2)
        ...
        BreakevenTrigger = round(float(tHigh[0]) + ((float(truncate(float(tHigh[0]),2))- float(truncate(float(tLow[0]),2))) * (int(RiskReward[0])* float(BreakevenVal[0]))),2)
...
#SHORT TRADES
    ...
    Currency = CCY1+CCY2
    TriggerID = OID  #Groups Parent and Child orders to single parent order ID
    Price = round(float(truncate(float(tLow[0]),2)) - 0.05,2)
    Strategy = "DailyBullBear"
    Bracket = "Entry"
    Status = "Submitted"
    parentId = OID
...
# create a profit take order of some kind
        ...
        Price = round(float(tLow[0]) - ((float(truncate(float(tHigh[0]),2))- float(truncate(float(tLow[0]),2))) * int(RiskReward[0])),2)
        ...
        StopLossvalue = round(float(truncate(float(tHigh[0]),4)) + 0.05,2)
        ...
        Breakeven = round(float(truncate(float(tLow[0]),2)) - 0.05, 2)
        logger.debug('Breakeven %s',Breakeven)
        query = ("UPDATE hasPosition SET Breakeven = " + str(Breakeven) + " WHERE CCY =\'" + CCY1 + CCY2+"\';")
        logger.debug('Query is %s',query)
        cur.execute(query)
        ...
        logger.debug('Query is %s',type(int(RiskReward[0])))
        logger.debug('Query is %s',type(float(BreakevenVal[0])))
        BreakevenTrigger = round(float(tLow[0]) - ((float(truncate(float(tHigh[0]),2))- float(truncate(float(tLow[0]),2))) * (int(RiskReward[0])* float(BreakevenVal[0]))),2)
        logger.debug('BreakevenTrigger %s', BreakevenTrigger)
        query = ("UPDATE hasPosition SET BreakevenTrigger = " + str(BreakevenTrigger) + " WHERE CCY =\'" + CCY1 + CCY2+"\';")
        logger.debug('Query is %s',query)
        cur.execute(query)
