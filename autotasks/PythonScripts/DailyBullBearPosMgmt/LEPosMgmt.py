...
logging.basicConfig(filename='pythonlogs\DailyPosMgmtLE' + str(datetime.date.today()) + '.txt', level=logging.DEBUG, format='%(asctime)s - %(levelname)s - %(message)s')
logger = logging.getLogger()
logger.setLevel(logging.DEBUG)

formatter = logging.Formatter('%(asctime)s - %(levelname)s - %(message)s')

fh = logging.FileHandler('DailyPosMgmtLE' + str(datetime.date.today()) + '.txt')
...
#Sets the Master Fill and Cancels other LE orders
    ...
    query = ("SELECT BB_EntryID FROM hasPosition where CCY = \'" + CCY1ES + CCY2ES + "\'")
    logger.debug('Query is %s', query)
    cur.execute(query)
    for (BB_EntryID) in cur:
        Order_entryES = BB_EntryID
        logger.debug('Entry ID is %s', Order_entryES[0])
    
    conn.cancelOrder(int(Order_entryES[0]))
    logger.debug('Canceled Order %s', Order_entryES[0] )
    ...
    query = ("UPDATE Orders SET Status = \'" + "Canceled" + "\' where idOrder = " + str(Order_entryES[0]) + ";")
    logger.debug('Query is %s', query)
    cur.execute(query)
    cnx.commit()
...
logger.debug('Time is %s', time.minute)
logger.debug('Time is %s', type(time.minute))
...
if (time.hour > 12 and time.minute > 14 and int(curCount[0]) == 0):
