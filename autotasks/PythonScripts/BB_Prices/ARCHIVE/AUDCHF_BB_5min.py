...
def reply_handler(msg):
    ...
    if float(test) != -1:
            ...
            cur = cnx.cursor()
            cur.execute("""Insert Into BB_AUDCHF (Date, Open, High, Low, Close) values(%s,%s,%s,%s,%s)""",(test5,truncate(float(test),4),truncate(float(test2),4),truncate(float(test3),4),truncate(float(test4),4)))
            cnx.commit()
...
#cnx = mysql.connector.connect(user='Scarlett01', password='scar01lett',host="serpdb01.cqxmc7cib5oh.us-east-1.rds.amazonaws.com", database='SERPDB01')
cnx = mysql.connector.connect(user=datalink.DB_User, password=datalink.DB_Pass,host=datalink.DB_Host, database=datalink.DB_Path)
query = ("Select round(stddev(Close),5) as StdDev from BB_AUDCHF where ID BETWEEN ((SELECT MAX(ID) as max_ID FROM BB_AUDCHF)-20) and (SELECT MAX(ID) as max_ID FROM BB_AUDCHF);")
cur = cnx.cursor()
cur.execute(query)
...
query = ("SELECT MAX(ID) as maxID FROM BB_AUDCHF")
cur = cnx.cursor()
cur.execute(query)
...
if maximumID[0] > 20:    
        ...
        query = ("Select round(Avg(Close),5) as Average from BB_AUDCHF where ID BETWEEN ((SELECT MAX(ID) as max_ID FROM BB_AUDCHF)-20) and (SELECT MAX(ID) as max_ID FROM BB_AUDCHF);")
        ...
        query = ("Update BB_AUDCHF set UpperBand = " + str(tUpperBand) + ", LowerBand = " + str(tLowerBand) + ", MidLine = " + str(BBAverage) + " where ID = " + str(maximumID[0]) + ";") 
        logger.debug('Query is %s', query)
        cur.execute(query)
        cnx.commit()
