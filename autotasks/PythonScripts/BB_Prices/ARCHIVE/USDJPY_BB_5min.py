...
def reply_handler(msg):
    ...
    if float(test) != -1:
            ...
            cur = cnx.cursor()
            cur.execute("""Insert Into BB_USDJPY (Date, Open, High, Low, Close) values(%s,%s,%s,%s,%s)""",(test5,truncate(float(test),2),truncate(float(test2),2),truncate(float(test3),2),truncate(float(test4),2)))
            cnx.commit()
...
conn.reqHistoricalData(6, qqq, '', '60 S', '5 mins', 'Midpoint', 1, 1) 
...
#cnx = mysql.connector.connect(user='Scarlett01', password='scar01lett',host="serpdb01.cqxmc7cib5oh.us-east-1.rds.amazonaws.com", database='SERPDB01')
cnx = mysql.connector.connect(user=datalink.DB_User, password=datalink.DB_Pass,host=datalink.DB_Host, database=datalink.DB_Path)
query = ("Select round(stddev(Close),5) as StdDev from BB_USDJPY where ID BETWEEN ((SELECT MAX(ID) as max_ID FROM BB_USDJPY)-20) and (SELECT MAX(ID) as max_ID FROM BB_USDJPY);")
cur = cnx.cursor()
cur.execute(query)
...
query = ("SELECT MAX(ID) as maxID FROM BB_USDJPY")
cur = cnx.cursor()
cur.execute(query)
for (maxID) in cur:
    maximumID = maxID   
...
if maximumID[0] > 20:    
        ...
        query = ("Select round(Avg(Close),5) as Average from BB_USDJPY where ID BETWEEN ((SELECT MAX(ID) as max_ID FROM BB_USDJPY)-20) and (SELECT MAX(ID) as max_ID FROM BB_USDJPY);")
        ...
        tUpperBand = truncate((float(BBAverage)  + (2 * float(tStdDev[0]))),2)
        logger.debug('BBAverage is %s', BBAverage)
        logger.debug('BBAverage is %s', type(BBAverage))
        logger.debug('std dev 2 is %s', (2 * float(tStdDev[0])))
        logger.debug('tUpperBand %s', tUpperBand)
        tLowerBand = truncate((float(BBAverage)  - (2 * float(tStdDev[0]))),2)
        logger.debug('Upper is . %s', tUpperBand)
        logger.debug('Lower is . %s', tLowerBand)
        BBAverage = truncate(BBAverage,2)
        
        query = ("Update BB_USDJPY set UpperBand = " + str(tUpperBand) + ", LowerBand = " + str(tLowerBand) + ", MidLine = " + str(BBAverage) + " where ID = " + str(maximumID[0]) + ";") 
        logger.debug('Query is %s', query)
        cur.execute(query)
        cnx.commit()
