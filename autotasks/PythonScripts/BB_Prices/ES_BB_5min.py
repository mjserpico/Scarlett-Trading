...
def reply_handler(msg):
    ...
    if float(test) != -1:
            ...
            cur = cnx.cursor()
            cur.execute("Insert Into " + Table + " (Date, Open, High, Low, Close) values(%s,%s,%s,%s,%s)",(test5,truncate(float(test),2),truncate(float(test2),2),truncate(float(test3),2),truncate(float(test4),2)))
            cnx.commit()
...
if maximumID[0] > 20:    
        ...
        for (Average) in cur:
            ...
            BBAverage = truncate(Average[0],2)  
