...
logging.basicConfig(filename='pythonlogs\CloseEOD6E' + str(datetime.date.today()) + '.txt', level=logging.DEBUG, format='%(asctime)s - %(levelname)s - %(message)s')
logger = logging.getLogger()
logger.setLevel(logging.DEBUG)

formatter = logging.Formatter('%(asctime)s - %(levelname)s - %(message)s')

fh = logging.FileHandler('CloseEOD6E' + str(datetime.date.today()) + '.txt')
...
#        conn.cancelOrder(int(Order_profit[0]))
        ...
        if longorshort[0] == 'Long':
            ORDER3 = close_long_order('Sell', 5, limit, transmit=True)
        ...
        if longorshort[0] == 'Short':        
            ORDER4 = close_short_order('Buy', 5, limit, transmit=True)
            CONTRACT = create_contract(symbol, secType, exchange, exchange, 'USD')
