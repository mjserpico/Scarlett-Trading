...
def make__short__order(action, qty, limit = None, profit_take=None, training_stop_percent=None, transmit=True, parentId=None):
        ...
        if limit == 2:
            ...
            if action == 'BUY':
                ...
                order.m_lmtPrice  = (float(tLow[0]) - (ProfitTargPips/10) - 0.2)
       	...
       	# ENTRY   A simple stop order
            ...
            if action == 'BUY':
                ...
                #logger.debug('Profit Pips is: %s', ProfitPips)
                #logger.debug('Risk Reward Ratio is: %s', RiskRewardRatio)
                order.m_lmtPrice  = (float(tLow[0]) - 0.2)
                stopPrice = truncate((float(tLow[0]) - 0.2),1)   #Stop Order -- Entry Price
            ...
            if action == 'SELL':
                ...
                order.m_lmtPrice  = (float(tHigh[0]) - 0.2)
                logger.debug('Limit Price: %s', order.m_lmtPrice)
                stopPrice = truncate((float(tLow[0]) - 0.2),1)
...
def make_order2(action, qty, limit = None, profit_take=None, training_stop_percent=None, transmit=True, parentId=None):
        ...
        if limit == 2:
            if action == 'SELL':
                #logger.debug('Profit Pips is %s', ProfitPips)
                order.m_lmtPrice  = truncate((float(tHigh[0]) + (ProfitTargPips[0]/10) + 0.2),1)
...
#                print(RiskRewardRatio)
       	...
       	# ENTRY   A simple stop order
            ...
            if action == 'BUY':
                ...
                lmtPrice = truncate((float(tHigh[0]) + 0.2),1)
                logger.debug('Buy Entry Price %s', lmtPrice)
                order.m_lmtPrice  = lmtPrice
                stopPrice = truncate((float(tHigh[0]) + 0.2),1)
                order.m_auxPrice = stopPrice;
                order.m_parentId = parentId
                order.m_account = masteraccount
                ...
                order.m_lmtPrice  = truncate((float(tHigh[0]) + 0.2),1)
                stopPrice = truncate((float(tHigh[0]) + 0.2),1)
                order.m_auxPrice = stopPrice;
                order.m_parentId = parentId
                order.m_account = masteraccount
...
if (CurrentPositions[0] < 6):  ##Removes Positions limits for BB Strat
    ...
    Price = float(truncate(float(tHigh[0]),1)) + 0.2  #Entry Price
    Strategy = "BB_STRAT"
    Bracket = "Entry"
    Status = "Submitted"
    parentId = OID
...
#LONGS    
    if (float(EntryPend[0]) == 0 and float(tClose[0]) > float(MovAvg[0]) and float(yClose[0]) < float(yOpen[0]) and float(tClose[0]) > float(tOpen[0]) and float(yLow[0]) < float(yLowerBand[0])): #float(tClose[0]) > float(MovAvg[0]) and float(yClose[0]) < float(yOpen[0]) and float(tClose[0]) > float(tOpen[0]) and float(yLow[0]) < float(yLowerBand[0])):
        ...
        #TWS Connection
        conn = Connection.create(port=4002, clientId=899)
        ...
        Currency = CCY1+CCY2
        TriggerID = OID
        Price = round(float(truncate(float(tHigh[0]),4)) + 0.2,1)
        Strategy = "BBSTRAT"
        Bracket = "Entry"
        Status = "Submitted"
        qty = int(PositionSize)
...
#Put STOPLOSS Value in hasPosition Table as Placeholder
        StopLossvalue = truncate(min((float(tLow[0]) - 0.2),(float(tHigh[0]) - 1)),1)  # Stop Price
...
# create a profit take order of some kind
        ...
        StopLossvalue = truncate(max((float(tHigh[0]) + 0.2),(float(tLow[0]) + 1)),1) #Either 2 pips above the High or 10 pips from the low, whichever is more
        query = ("UPDATE hasPosition SET BB_StopLossVal = " + str(StopLossvalue) + " WHERE CCY =\'" + CCY1 + CCY2+"\';")
        cur.execute(query)
        logger.debug('hasPosition query is %s',query)
        cnx.commit()
