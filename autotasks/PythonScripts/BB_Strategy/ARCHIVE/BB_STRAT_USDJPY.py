...
def make__short__order(action, qty, limit = None, profit_take=None, training_stop_percent=None, transmit=True, parentId=None):
        ...
        if limit == 2:
            ...
            if action == 'BUY':
                ...
                order.m_lmtPrice  = (float(tLow[0]) - (ProfitTargPips/10000) - 0.0002)
       	...
       	# ENTRY   A simple stop order
            ...
            if action == 'BUY':
                ...
                logger.debug('Profit Pips is: %s', ProfitPips)
                logger.debug('Risk Reward Ratio is: %s', RiskRewardRatio)
                order.m_lmtPrice  = (float(tLow[0]) - 0.02)
                stopPrice = (float(tLow[0]) - 0.02)
            ...
            if action == 'SELL':
                ...
                order.m_lmtPrice  = (float(tHigh[0]) - 0.02)
				logger.debug('Limit Price: %s', order.m_lmtPrice)
                stopPrice = (float(tLow[0]) - 0.02)
...
def make_order2(action, qty, limit = None, profit_take=None, training_stop_percent=None, transmit=True, parentId=None):
        ...
        if limit == 2:
            if action == 'SELL':
                logger.debug('Profit Pips is %s', ProfitPips)
                order.m_lmtPrice  = (float(tHigh[0]) + (ProfitTargPips[0]/10000) + 0.02)
                logger.debug('In limit order of make order2')
                logger.debug('Limit Price is is %s', order.m_lmtPrice)
                logger.debug('Risk Reward Ratio is %s', RiskRewardRatio)
                order.m_orderType = 'LMT'
                order.m_account = masteraccount
                order.m_transmit = transmit
...
#                print(RiskRewardRatio)
       	...
       	# ENTRY   A simple stop order
            ...
            if action == 'BUY':
                ...
                lmtPrice = truncate((float(tHigh[0]) + 0.02),2)
                logger.debug('Buy Entry Price %s', lmtPrice)
                order.m_lmtPrice  = lmtPrice
                stopPrice = truncate((float(tHigh[0]) + 0.02),2)
                order.m_auxPrice = stopPrice;
                order.m_parentId = parentId
                order.m_account = masteraccount
                ...
                order.m_lmtPrice  = truncate((float(tHigh[0]) + 0.02),2)
                stopPrice = truncate((float(tHigh[0]) + 0.02),2)
                order.m_auxPrice = stopPrice;
                order.m_parentId = parentId
                order.m_account = masteraccount
...
query = ("SELECT max(ID) from BB_" + CCY1+CCY2+ ";")
...
# '" + str(int(BB_ID)) + "\';"
query = ("SELECT Open from BB_" + CCY1 + CCY2 + " where ID = " + str(BB_ID[0]) + ";")
...
query = ("SELECT High from BB_" + CCY1 + CCY2 + " where ID = " + str(BB_ID[0]) + ";")
cur.execute(query)
for (High) in cur:
    tHigh = High
...
query = ("SELECT Low from BB_" + CCY1 + CCY2 + " where ID = " + str(BB_ID[0]) + ";")
cur.execute(query)
for (Low) in cur:
   tLow = Low
...
query = ("SELECT Close from BB_" + CCY1 + CCY2 + " where ID = " + str(BB_ID[0]) + ";")
cur.execute(query)
for (Close) in cur:
    tClose = Close
...
query = ("SELECT UpperBand from BB_" + CCY1 + CCY2 + " where ID = " + str(BB_ID[0]) + ";")
cur.execute(query)
...
query = ("SELECT LowerBand from BB_" + CCY1 + CCY2 + " where ID = " + str(BB_ID[0]) + ";")
cur.execute(query)
...
query = ("SELECT MidLine from BB_" + CCY1 + CCY2 + " where ID = " + str(BB_ID[0]) + ";")
cur.execute(query)
for (MidLine) in cur:
    tMidLine = MidLine
...
query = ("SELECT (max(ID)-1) from BB_" + CCY1+CCY2+ ";")
...
query = ("SELECT Open from BB_" + CCY1 + CCY2 + " where ID = " + str(yBB_ID[0]) + ";")
logger.debug('query is %s', query)
cur.execute(query)
for (Open) in cur:
    yOpen = Open
...
query = ("SELECT High from BB_" + CCY1 + CCY2 + " where ID = " + str(yBB_ID[0]) + ";")
logger.debug('query is %s', query)
cur.execute(query)
for (High) in cur:
    yHigh = High
...
query = ("SELECT Low from BB_" + CCY1 + CCY2 + " where ID = " + str(yBB_ID[0]) + ";")
logger.debug('query is %s', query)
cur.execute(query)
for (Low) in cur:
    yLow = Low
...
query = ("SELECT Close from BB_" + CCY1 + CCY2 + " where ID = " + str(yBB_ID[0]) + ";")
logger.debug('query is %s', query)
cur.execute(query)
for (Close) in cur:
    yClose = Close
...
query = ("SELECT UpperBand from BB_" + CCY1 + CCY2 + " where ID = " + str(yBB_ID[0]) + ";")
logger.debug('query is %s', query)
cur.execute(query)
for (UpperBand) in cur:
...
query = ("SELECT LowerBand from BB_" + CCY1 + CCY2 + " where ID = " + str(yBB_ID[0]) + ";")
logger.debug('query is %s', query)
cur.execute(query)
for (LowerBand) in cur:
...
query = ("SELECT MidLine from BB_" + CCY1 + CCY2 + " where ID = " + str(yBB_ID[0]) + ";")
logger.debug('query is %s', query)
cur.execute(query)
for (MidLine) in cur:
    yMidLine = MidLine
...
if (CurrentPositions[0] < 1000000):  ##Removes Positions limits for BB Strat
    ...
    Price = float(truncate(float(tHigh[0]),4)) + 0.02  #Entry Price
    Strategy = "BB_STRAT"
    Bracket = "Entry"
    Status = "Submitted"
    parentId = OID
...
#LONGS    
    if (float(EntryPend[0]) == 0 and float(tClose[0]) > float(MovAvg[0]) and float(yClose[0]) < float(yOpen[0]) and float(tClose[0]) > float(tOpen[0]) and float(yLow[0]) < float(yLowerBand[0])): #float(tClose[0]) > float(MovAvg[0]) and float(yClose[0]) < float(yOpen[0]) and float(tClose[0]) > float(tOpen[0]) and float(yLow[0]) < float(yLowerBand[0])):
        ...
        Currency = CCY1+CCY2
        TriggerID = OID
        Price = round(float(truncate(float(tHigh[0]),4)) + 0.02,4)
        Strategy = "BBSTRAT"
        Bracket = "Entry"
        Status = "Submitted"
        qty = int(PositionSize)
...
#Put STOPLOSS Value in hasPosition Table as Placeholder
        StopLossvalue = min((float(tLow[0]) - 0.02),(float(tHigh[0]) - 0.1)) 
...
#SHORT TRADES 
    ...
    Currency = CCY1+CCY2
    TriggerID = OID  #Groups Parent and Child orders to single parent order ID
    Price = round(float(truncate(float(tLow[0]),4)) - 0.02,4)
    Strategy = "BB_STRAT"
    Bracket = "Entry"
    Status = "Submitted"
    parentId = OID
...
# create a profit take order of some kind
        ...
        StopLossvalue = max((float(tHigh[0]) + 0.02),(float(tLow[0]) + 0.1)) 
        query = ("UPDATE hasPosition SET BB_StopLossVal = " + str(StopLossvalue) + " WHERE CCY =\'" + CCY1 + CCY2+"\';")
        cur.execute(query)
        logger.debug('hasPosition query is %s',query)
        cnx.commit()
...
logging.debug('end %s%s',CCY1,CCY2)