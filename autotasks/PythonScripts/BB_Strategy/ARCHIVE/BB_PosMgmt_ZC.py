...
def make_trail(action, qty, limit = None, transmit=True):
        ...
        if action == 'Sell':
            ...
            stopPrice = truncate(float(stopVal[0]),1)
            EntPrice = truncate(float(EntryPrice[0]),1)
            logger.debug('In Sell action. TRAIL LIMIT stop price is %s', stopPrice)
            logger.debug('In Sell action. AUX trail amt price is %s', (float(stopPrice[0]) - float(EntPrice[0])))
            auxvalue = truncate(max((float(stopPrice[0]) - float(EntPrice[0])),(0.001)),1)
            logger.debug('aux value is %s', auxvalue)
            order.m_auxPrice = auxvalue #max((float(stopPrice[0]) - float(EntPrice[0])),0.001)
        ...
        if action == 'Buy':
            #order.m_lmtPrice  = limit + int(np.around((limit*profit_take_percent)/100.0, 5)/0.00005)*0.00005
            #order.m_lmtPrice  = truncate(float(stopVal[0]),4)
            stopPrice = truncate(float(stopVal[0]),1) #truncate(float(stopVal[0]),4)
            EntPrice = truncate(float(EntryPrice[0]),1)
            logger.debug('In Buy action. TRAIL LIMIT stop price is %s', stopPrice)
            logger.debug('In Buy action. AUX trail amt price is %s', stopPrice)
            order.m_auxPrice = (EntPrice - stopPrice);  #Aux Price is Trail AMT
            order.m_trailStopPrice = stopPrice
            order.m_account = masteraccount
            #order.m_parentId = parentId
            logger.debug('In Buy action. account is %s', order.m_account)
...
@author: Michael
"""
if (inPos == 1 and curStopLossFlag[0] == "0"):
