...
logging.basicConfig(filename='pythonlogs\BBPosMgmtNoGlobex' + str(datetime.date.today()) + '.txt', level=logging.DEBUG, format='%(asctime)s - %(levelname)s - %(message)s')
logger = logging.getLogger()
logger.setLevel(logging.DEBUG)

formatter = logging.Formatter('%(asctime)s - %(levelname)s - %(message)s')

fh = logging.FileHandler('BBPosMgmtNoGlobex' + str(datetime.date.today()) + '.txt')
...
def make__short__order(action, qty, limit = None, profit_take=None, training_stop_percent=None, transmit=True, parentId=None):
        ...
        order.m_ocaGroup = 3
        order.ocaType = 1
        profitlimitprice = truncate((float(profitVal[0])), 3)
...
def make_long_order(action, qty, limit = None, profit_take=None, training_stop_percent=None, transmit=True, parentId=None):
        ...
        order.m_lmtPrice  = truncate((float(profitVal[0])), 3)
...
qqq.m_localSymbol = CCY1+CCY2 + datalink2.monthcode + "7";
qqq.m_secType = 'FUT'  
...
if (time.hour > 14 and int(curCount[0]) == 0):
