...
logging.basicConfig(filename='pythonlogs\BBSTRATEGYNoGlobex' + str(datetime.date.today()) + '.txt', level=logging.DEBUG, format='%(asctime)s - %(levelname)s - %(message)s')
logger = logging.getLogger()
logger.setLevel(logging.DEBUG)

formatter = logging.Formatter('%(asctime)s - %(levelname)s - %(message)s')

fh = logging.FileHandler('BBSTRATEGYNoGlobex' + str(datetime.date.today()) + '.txt')
...
def make__short__order(action, qty, limit = None, profit_take=None, training_stop_percent=None, transmit=True, parentId=None):
        ...
        if limit == 2: # Profit Target
            ...
            if action == 'BUY':
                ...
                profitlimitprice = truncate((float(tLow[0]) - 1.75), 3)
...
#                order.m_transmit = transmit
            ...
            if action == 'SELL':
                ...
                lmtPrice = truncate((float(tLow[0]) - 0.10),3)
                logger.debug('Buy Stop Limit"Limit" Price or better to buy %s', lmtPrice)
                order.m_lmtPrice  = lmtPrice
                logger.debug('Low Price is::: %s', (float(tLow[0])))
                stopPrice = round(float(truncate(float(tLow[0]),3)) - 0.075,3)
...
def make_long_order(action, qty, limit = None, profit_take=None, training_stop_percent=None, transmit=True, parentId=None):
        ...
        if limit == 2:  #Profit Target 
            if action == 'SELL':
                #logger.debug('Profit Pips is %s', ProfitPips)
                order.m_lmtPrice  = truncate((float(tHigh[0]) + 1.75), 3)
...
#                print(RiskRewardRatio)
       	...
       	# ENTRY   A simple stop order
            ...
            if action == 'BUY':
                ...
                lmtPrice = truncate((float(tHigh[0]) + 0.10),3)
                logger.debug('Buy Stop Limit"Limit" Price or better to buy %s', lmtPrice)
                order.m_lmtPrice  = lmtPrice
                stopPrice = truncate((float(tHigh[0]) + 0.075),3)
...
def create_contract(symbol, sec_type, exch, prim_exch, curr):
    ...
    contract.m_localSymbol = CCY1+CCY2 + datalink2.monthcode + "7";
...
if (CurrentPositions[0] < 1000000):  ##Removes Positions limits for BB Strat
    ...
    Price = float(truncate(float(tHigh[0]),3)) + 0.075  #Entry Price
    Strategy = "BB_STRAT"
    Bracket = "Entry"
    Status = "Submitted"
    parentId = OID
...
#LONGS    
    if (float(EntryPend[0]) == 0 and float(tClose[0]) > float(MovAvg[0]) and float(yClose[0]) < float(yOpen[0]) and float(tClose[0]) > float(tOpen[0]) and float(yLow[0]) < float(yLowerBand[0])): #float(tClose[0]) > float(MovAvg[0]) and float(yClose[0]) < float(yOpen[0]) and float(tClose[0]) > float(tOpen[0]) and float(yLow[0]) < float(yLowerBand[0])):
        ...
        Currency = CCY1+CCY2
        TriggerID = OID
        Price = round(float(truncate(float(tHigh[0]),4)) + 0.075,3)
        Strategy = "BBSTRAT"
        Bracket = "Entry"
        Status = "Submitted"
        qty = int(PositionSize)
...
#Create ENTRY Stp Buy Order 2 pips above High
        ...
        cur.execute(query)
        cnx.commit()
        logger.debug('hasPosition Entry test@@ Price set to %s ', Price)
...
#        logger.debug('idOrder is %s ', idOrder)
 #REMOVE PROFIT TARGET IN STRAT AND MOVE TO POSMGTM AS PART OF OCA WITH STOP LOSS       
        ...
        Price = (float(tHigh[0]) + 1.75)  # Keep this to ensure Orders shows proper Profit Target
        #cur.execute("""Insert Into Orders (idOrder, Date, Time, Currency, TriggerID, Price, Strategy, Bracket, Status) values(%s,%s,%s,%s,%s,%s,%s,%s,%s)""",(idOrder, Date, time, Currency, TriggerID, Price, Strategy, Bracket, Status))
        #cur.execute(query)
...
#Put STOPLOSS Value in hasPosition Table as Placeholder
        StopLossvalue = truncate(min((float(tLow[0]) - 0.05),(float(tHigh[0]) - 0.5)),3)  # Stop Price
...
#SHORT TRADES 
    ...
    Currency = CCY1+CCY2
    TriggerID = OID  #Groups Parent and Child orders to single parent order ID
    Price = round(float(truncate(float(tLow[0]),2)) - 0.075,3)
    Strategy = "BB_STRAT"
    Bracket = "Entry"
    Status = "Submitted"
    parentId = OID
...
#        idOrder = OID
        ...
        StopLossvalue = truncate(max((float(tHigh[0]) + 0.05),(float(tLow[0]) + 0.5)),3) #Either 2 pips above the High or 10 pips from the low, whichever is more
        query = ("UPDATE hasPosition SET BB_StopLossVal = " + str(StopLossvalue) + " WHERE CCY =\'" + CCY1 + CCY2+"\';")
        cur.execute(query)
        logger.debug('hasPosition query is %s',query)
        cnx.commit()
