...
logging.basicConfig(filename='pythonlogs\CloseEODHE' + str(datetime.date.today()) + '.txt', level=logging.DEBUG, format='%(asctime)s - %(levelname)s - %(message)s')
logger = logging.getLogger()
logger.setLevel(logging.DEBUG)

formatter = logging.Formatter('%(asctime)s - %(levelname)s - %(message)s')

fh = logging.FileHandler('CloseEODHE' + str(datetime.date.today()) + '.txt')
...
def reply_handler_pnl(msg):
    test0 = msg
    logger.debug('Reply Handler PnL is %s', test0) 
...
def make__short__order(action, qty, limit = None, profit_take=None, training_stop_percent=None, transmit=True, parentId=None):
        ...
        logger.debug('Profit Target Limit Order Price : %s', profitlimitprice)
...
def close_short_order(action, qty, limit = None, profit_take=None, training_stop_percent=None, transmit=True, parentId=None):
        logger.debug('In Short Order Function')    
        order = Order()
        logger.debug('action is %s', action)  
        logger.debug('qty is %s', qty) 
        ...
        #order.ocaType = 1
       # profitlimitprice = truncate((float(profitVal[0])), 2)
        logger.debug('Close Short MKT Price ')
        #order.m_lmtPrice  = profitlimitprice
...
def close_long_order(action, qty, limit = None, profit_take=None, training_stop_percent=None, transmit=True, parentId=None):
        order = Order()
        order.m_action = action
        order.m_totalQuantity = qty
        #order.m_ocaGroup = 1
        #order.ocaType = 1
        order.m_tif = "GTC"  #All orders are GTC by default with NO TIME LIMIT to auto cancel
        logger.debug('In Close Long Order Function')
        #order.m_lmtPrice  = truncate((float(profitVal[0])), 2)
        logger.debug('MKT Order Price')
        order.m_orderType = 'MKT'
        order.m_account = masteraccount
        order.m_transmit = transmit
        logger.debug('transmitted Close LONG order')
        return order 
...
#conn.register(reply_handler_Status, 'OrderStatus')
    ...
    conn.register(reply_handler_pnl, 'PnL')
    time.sleep(1)
#conn.register(reply_handler_contract, 'Contract')     
    ...
    qqq.m_localSymbol = CCY1+CCY2 + datalink2.monthcode + "8";
    qqq.m_secType = 'FUT'  
...
logger.debug('In Position is  %s', inPos)
logger.debug('In Position is  %s', type(inPos)) 
logger.debug('In Position is  %s', type(curCount[0])) 
...
if (int(curCount[0]) == 1 and int(inPos) == 1):
        logger.debug('Closing Positions for the day')
        #logger.debug('Stop Order ID is %s', Order_stop[0])
        #conn.cancelOrder(Order_stop[0])
        logger.debug('Canceling outstanding Stop and Profit Target Orders')
...
#        conn.cancelOrder(int(Order_profit[0]))
        ...
        for (direction) in cur:
            longorshort = BB_direction
            logger.debug('Stop loss value is %s', longorshort)
        ...
        OIDend = maxID[0] + 1
        logger.debug('close ID is %s', OIDend)
        if longorshort[0] == 'Long':
            ORDER3 = close_long_order('Sell', 1, limit, transmit=True)
            logger.debug('Long Order created')
            CONTRACT = create_contract(symbol, secType, exchange, exchange, 'USD')
            logger.debug('Long Contract created')
            conn.placeOrder(OIDend, CONTRACT, ORDER3)
            cur.execute("""Insert Into Orders (idOrder, Date, Time, Currency, TriggerID, Price, Strategy, Bracket, Status) values(%s,%s,%s,%s,%s,%s,%s,%s,%s)""",(OIDend, today, timer, Currency, TriggerID, 'MKT', Strategy, 'Close', Status))
            cnx.commit() 

        
            #In this section when a short trade in effect
        if longorshort[0] == 'Short':        
            ORDER4 = close_short_order('Buy', 1, limit, transmit=True)
            CONTRACT = create_contract(symbol, secType, exchange, exchange, 'USD')
            conn.placeOrder(OIDend, CONTRACT, ORDER4)
            cur.execute("""Insert Into Orders (idOrder, Date, Time, Currency, TriggerID, Price, Strategy, Bracket, Status) values(%s,%s,%s,%s,%s,%s,%s,%s,%s)""",(OIDend, today, timer, Currency, TriggerID, 'MKT', Strategy, 'Close', Status))
            cnx.commit()
...
#if (time.hour > 16 and int(curCount[0]) == 0):
