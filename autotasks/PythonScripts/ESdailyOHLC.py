...
logger.debug('Starting ESDailyOHLC')
...
#20 period Moving Average
            ...
            query = ("SELECT round(Avg(Close),2) as Avg from " + CCY1 + CCY2 + " where ID BETWEEN " +  str(ID2[0]) + " AND " +  str(ID1[0]) + ";")
            logger.debug('Query is %s', query)
            cur.execute(query)
            for (Avg) in cur:
...
while Flag == 0:            
    conn = Connection.create(port=4002, clientId=887)
    conn.connect()
    ...
    conn.disconnect()
logger.debug('Finished ES Daily OHLC')