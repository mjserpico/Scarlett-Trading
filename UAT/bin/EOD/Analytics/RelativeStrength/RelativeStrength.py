...
for row in csv_f:
        ...
        #5 days back        
            ...
            query = ("SELECT (max(ID)-5) from " + Table)
            logger.debug('Query is %s', query)
            cur.execute(query)
            for (ID) in cur:
                ID5index = ID
                logger.debug('ID5index is %s', ID5index)
            ...
            for (ID) in cur:
                ID5ticker = ID
                logger.debug('ID5ticker is %s', ID5ticker)

            
#Close from 5 Days Back
            
            query = ("SELECT Close from " + Table + " where ID = " + str(ID5index[0]) + ";")
            cur.execute(query)
            for (Close) in cur:
                indexfiveClose = Close
                logger.debug('indexfiveClose is %s', indexfiveClose[0])
                
            query = ("SELECT Close from " + Ticker[0] + " where ID = " + str(ID5ticker[0]) + ";")
            cur.execute(query)
            for (Close) in cur:
                tickerfiveClose = Close
                logger.debug('tickerfiveClose is %s', tickerfiveClose[0])
...
#Calculate the Percent Return over 5 days for Ticker
            returnsStock = ((float(tickerClose[0]) - float(tickerfiveClose[0])) / float(tickerfiveClose[0])*100)
            returnsStock2 = truncate(returnsStock,2)
            logger.debug('returnsStock is %s', returnsStock2)  
...
#Relative Strength      
            ...
            query = ("UPDATE hasPosition SET RelativeStrength = " + str(RelStr2) + " where CCY =\'" + Ticker[0] +"\';")
            logger.debug('Query is %s', query)
            cur.execute(query)
            cnx.commit()        
