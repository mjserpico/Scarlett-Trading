...
def reply_handler(msg):
    ...
    if float(test2) != -1:
        gap_close = msg.high
    test5 = msg.volume
    logger.debug('open %s', test)
    logger.debug('high %s', test2)
    logger.debug('low %s', test3)
    logger.debug('volume %s', test5)
    logger.debug('GAP CLOSE %s', gap_close)
    global Flag
    Flag = 1
...
def make_long_order(action, qty, limit = None, profit_take=None, training_stop_percent=None, transmit=True, parentId=None):
        ...
        lmtPrice = float(truncate((float(yHigh) + (float(ATRentrymultiple[0])*float(ATRVal[0]))),2))
        ...
        stopPrice = float(truncate((float(yHigh) + (float(ATRentrymultiple[0])*float(ATRVal[0]))),2))
...
for row in csv_f:
    Ticker = row
    logger.debug('Ticker is before pulling price %s', Ticker[0])
    gap_flag = 0
    logger.debug('Flag before loop set to %s', Flag)
    while Flag == 0:
        ...
        conn.reqHistoricalData(1, qqq, '', '1 D', '15 mins', 'MIDPOINT', 0, 1)
        logger.debug('Returned from Reply Handler')
        time.sleep(0.5) #give IB time to send us messages
        logger.debug('Disconnecting from Server')
        conn.disconnect()
    
    
    Flag = 0
    logger.debug('Ticker is after pulling price %s', Ticker[0])
    logger.debug('GAP_CLOSE is %s', gap_close)
    logger.debug('Flag out of loop set to %s', Flag)
   ...
   #AvgDaillyVolume   
    query = ("SELECT AvgVolume from hasPosition where CCY = \'" + Ticker[0] +"\';")
    cur.execute(query)
    for (AvgVolume) in cur:
        AvgVol = AvgVolume
        logger.debug('AvgVolume is %s', AvgVol)
    ...
    logger.debug('gap_close is %s', gap_close)
    yHigh = gap_close
    logger.debug('yesterday High is---Now actually 15 minute morning range %s', yHigh)
    ...
    Price = float(truncate((float(yHigh) + (float(ATRentrymultiple[0])*float(ATRVal[0]))),2))
    logger.debug('Projected Entry Price is %s', Price)
    logger.debug('Projected Entry Price type is %s', type(Price))
    logger.debug('gap flag is %s', gap_flag)
    logger.debug('gap flag is %s', type(gap_flag))
    logger.debug('gap close is %s', gap_close)
    logger.debug('gap close is %s', type(gap_close))
    #if(gap_close > Price):
    #    gap_flag = 1
    ...
    if (float(EntryPend[0]) == 0 and float(yClose[0]) > float(MovAvg[0]) and float(AvgVol[0] > 500000): # and gap_flag == 0
        ...
        Currency = 'USD'
        TriggerID = idOrder
        logger.debug('yHigh is %s',type(yHigh))
