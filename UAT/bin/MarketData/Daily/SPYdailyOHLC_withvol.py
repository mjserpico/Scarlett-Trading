...
#ID for MovAvg length start point                
            ...
            query = ("SELECT Close from " + CCY1 + CCY2 + " where Date = '12/31/2018';")
            cur.execute(query)
            for (Close) in cur:
                ytdClose = Close
                logger.debug('ytdClose is %s', ytdClose[0])
...
#1 Day Return
 ...
 #YTD Return
            CloseReturn = float(tClose[0])
            ytdCloseReturn = float(ytdClose[0])
            logger.debug('ytdClose is %s', ytdClose[0])
            logger.debug('Close is %s', tClose[0])
            YTDreturns = round(((CloseReturn / ytdCloseReturn) - 1) * 100,2)
            logger.debug('YTDReturn is %s', YTDreturns)
            
            query = ("UPDATE " + CCY1 + CCY2 + " SET YTDReturn = " + str(YTDreturns) + " where ID = " + str(ID1[0]) +";")
            logger.debug('Query is %s', query)
            cur.execute(query)
            cnx.commit()
...
#Sharpe Ratio is always 1 on index 
            query = ("UPDATE "+ Table +" SET SharpeRatio = '1' where ID =\'" + str(ID1[0]) +"\';")
            logger.debug('Query is %s', query)
            print(query)
            cur.execute(query)
            cnx.commit()
...
#Max Drawdown of index            
            ...
            CurDrawdown = Maxval - Minval 
            
            #Compare the current drawdown to the maximm previous drawdown then insert the higher value
            query = ("Select max(MaxDrawDown) as Maximum from" + CCY1 + CCY2 + ";")
            logger.debug('Query is %s', query)
            cur.execute(query)
            for (Maximum) in cur:
                Maxval = Maximum  #Final Moving Average Value
