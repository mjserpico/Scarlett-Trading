...
def reply_handler(msg):
    ...
    if float(test) != -1:
            ...
            cur = cnx.cursor()
            cur.execute("Insert Into "+ Table + """(Date, Open, High, Low, Close, Volume) values(%s,%s,%s,%s,%s,%s)""",(time.strftime("%m/%d/%Y"),float(test),float(test2),float(test3),float(test4),float(test5)))
            cnx.commit()
...
#Daily Average Volume             
            query = ("SELECT Avg(Volume) as AvgVol from " + CCY1 + CCY2 + " where ID BETWEEN " +  str(ID4[0]) + " AND " +  str(ID1[0]) + ";")
            logger.debug('Query is %s', query)
            print(query)
            cur.execute(query)
            for (AvgVol) in cur:
                VolumeDaily = AvgVol   #Final Moving Average Value
                logger.debug('Average daily volume is is %s', VolumeDaily)

##Puts Avg dailt volume in hasPosition Table for Reference with intraday strategies
            query = ("UPDATE hasPosition SET AvgVolume = " + str(VolumeDaily[0]) + " where CCY =\'" + CCY1 + CCY2 +"\';")
            logger.debug('Query is %s', query)
            cur.execute(query)
            print(query)
            cnx.commit()
