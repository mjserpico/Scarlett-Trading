...
logging.basicConfig(filename='emailConnection' + str(datetime.date.today()) + '.txt', level=logging.DEBUG, format='%(asctime)s - %(levelname)s - %(message)s')
logger = logging.getLogger()
logger.setLevel(logging.DEBUG)

formatter = logging.Formatter('%(asctime)s - %(levelname)s - %(message)s')

fh = logging.FileHandler('emailConnection' + str(datetime.date.today()) + '.txt')
...
messagetext = "Connection Status \n" + str(bal[0])
subjecttext = "Connection Status"
