...
logging.basicConfig(filename='resetConnection' + str(datetime.date.today()) + '.txt', level=logging.DEBUG, format='%(asctime)s - %(levelname)s - %(message)s')
logger = logging.getLogger()
logger.setLevel(logging.DEBUG)

formatter = logging.Formatter('%(asctime)s - %(levelname)s - %(message)s')

fh = logging.FileHandler('resetConnection' + str(datetime.date.today()) + '.txt')
...
#            logger.debug('Ran insert statement')
            ...
            Flag = 1
            print(Flag)
            logger.debug('Flag set to 0')
            query = ("UPDATE RiskParameters SET RiskParametersValue='0' WHERE idRiskParameters='8';")
