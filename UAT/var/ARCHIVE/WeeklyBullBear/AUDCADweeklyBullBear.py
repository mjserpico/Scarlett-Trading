...
logger = logging.getLogger('AUDCAD')
hdlr = logging.FileHandler('log/AUDCAD' + str(today) + '.log')
formatter = logging.Formatter('%(asctime)s %(levelname)s %(message)s')
hdlr.setFormatter(formatter)
logger.addHandler(hdlr) 
#logger.setLevel(logging.WARNING)
logger.error('Starting AUDCAD')
...
if (CurrentPositions < MaxPositions):
    logger.error('There is an available position slot for AUDCAD')
    symbol   = CCY1
    symbol2 = CCY2
    secType  = 'CASH'
    exchange = 'IDEALPRO'
