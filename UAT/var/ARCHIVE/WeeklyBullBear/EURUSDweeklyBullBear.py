...
logger = logging.getLogger('EURUSD')
hdlr = logging.FileHandler('log/EURUSD' + str(today) + '.log')
formatter = logging.Formatter('%(asctime)s %(levelname)s %(message)s')
hdlr.setFormatter(formatter)
logger.addHandler(hdlr) 
#logger.setLevel(logging.WARNING)
logger.error('Starting EURUSD')
...
if (CurrentPositions < MaxPositions):
    logger.error('There is an available position slot for EURUSD')
    symbol   = CCY1
    symbol2 = CCY2
    secType  = 'CASH'
    exchange = 'IDEALPRO'
