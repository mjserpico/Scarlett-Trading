...
logger = logging.getLogger('AUDJPY')
hdlr = logging.FileHandler('log/AUDJPY' + str(today) + '.log')
formatter = logging.Formatter('%(asctime)s %(levelname)s %(message)s')
hdlr.setFormatter(formatter)
logger.addHandler(hdlr) 
#logger.setLevel(logging.WARNING)
logger.error('Starting AUDJPY')
...
if (CurrentPositions < MaxPositions):
    logger.error('There is an available position slot for AUDJPY')
    symbol   = CCY1
    symbol2 = CCY2
    secType  = 'CASH'
    exchange = 'IDEALPRO'
