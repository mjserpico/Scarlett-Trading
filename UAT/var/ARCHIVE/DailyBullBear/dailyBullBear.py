# -*- coding: utf-8 -*-
"""
Master DAILY BULL BEAR.

IMPROVED TO RUN ALL CONTRACTS FROM ONE SCRIPT.  REFERENCES PRODUCT STATIC DTAT for contract details
"""
#
...
def create_contract(symbol, sec_type, exch, prim_exch, curr):
    ...
    contract.m_localSymbol = "E7" + datalink.monthcode + "7";
...
if (CurrentPositions < MaxPositions):
    ...
    Price = round(float(truncate(float(tHigh[0]),4)) + 0.0005,4)
...
# create a profit take order of some kind
        limit = 2
        Price = round(float(tHigh[0]) + ((float(truncate(float(tHigh[0]),4))- float(truncate(float(tLow[0]),4))) * int(RiskReward[0])),4)
        ...
        BreakevenTrigger = round(float(tHigh[0]) + ((float(truncate(float(tHigh[0]),4))- float(truncate(float(tLow[0]),4))) * (int(RiskReward[0])* float(BreakevenVal[0]))),4)
...
#SHORT TRADES
    ...
    Currency = CCY1+CCY2
    TriggerID = OID  #Groups Parent and Child orders to single parent order ID
    Price = round(float(truncate(float(tLow[0]),4)) - 0.0005,4)
    Strategy = "DailyBullBear"
    Bracket = "Entry"
    Status = "Submitted"
    parentId = OID
...
# create a profit take order of some kind
        ...
        Price = round(float(tLow[0]) - ((float(truncate(float(tHigh[0]),4))- float(truncate(float(tLow[0]),4))) * int(RiskReward[0])),4)
        ...
        StopLossvalue = round(float(truncate(float(tHigh[0]),4)) + 0.0005,4)
        ...
        Breakeven = round(float(truncate(float(tLow[0]),4)) - 0.0005,4)
        logger.debug('Breakeven %s',Breakeven)
        query = ("UPDATE hasPosition SET Breakeven = " + str(Breakeven) + " WHERE CCY =\'" + CCY1 + CCY2+"\';")
        logger.debug('Query is %s',query)
        cur.execute(query)
        ...
        BreakevenTrigger = round(float(tLow[0]) -  ((float(truncate(float(tHigh[0]),4))- float(truncate(float(tLow[0]),4))) * (int(RiskReward[0])* float(BreakevenVal[0]))),4)
        logger.debug('BreakevenTrigger %s', BreakevenTrigger)
        query = ("UPDATE hasPosition SET BreakevenTrigger = " + str(BreakevenTrigger) + " WHERE CCY =\'" + CCY1 + CCY2+"\';")
        logger.debug('Query is %s',query)
        cur.execute(query)
