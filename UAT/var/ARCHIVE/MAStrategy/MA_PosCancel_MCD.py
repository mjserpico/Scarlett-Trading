...
logging.basicConfig(filename='pythonlogs\MAPosMgmt' + str(datetime.date.today()) + '.txt', level=logging.DEBUG, format='%(asctime)s - %(levelname)s - %(message)s')
logger = logging.getLogger()
logger.setLevel(logging.DEBUG)

formatter = logging.Formatter('%(asctime)s - %(levelname)s - %(message)s')

fh = logging.FileHandler('MAPosMgmt' + str(datetime.date.today()) + '.txt')
...
def make_trail(action, qty, limit = None, transmit=True):
        ...
        if action == 'Buy':
            ...
            stopPrice = 0.005 #truncate(float(stopVal[0]),4)
            order.m_auxPrice = stopPrice;
            order.m_account = masteraccount
            #order.m_parentId = parentId
