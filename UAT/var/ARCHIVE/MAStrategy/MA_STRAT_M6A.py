...
logging.basicConfig(filename='pythonlogs\MASTRATEGY' + str(datetime.date.today()) + '.txt', level=logging.DEBUG, format='%(asctime)s - %(levelname)s - %(message)s')
logger = logging.getLogger()
logger.setLevel(logging.DEBUG)

formatter = logging.Formatter('%(asctime)s - %(levelname)s - %(message)s')

fh = logging.FileHandler('MASTRATEGY' + str(datetime.date.today()) + '.txt')
...
logger.debug('Starting MA_STRAT %s %s', CCY1,CCY2)
...
#LONGS    
    if (float(EntryPend[0]) == 0 and float(tClose[0]) > float(MovAvg[0]) and float(yClose[0]) > float(tMidLine[0]) and float(tClose[0]) > float(tMidLine[0])): #float(tClose[0]) > float(MovAvg[0]) and float(yClose[0]) < float(yOpen[0]) and float(tClose[0]) > float(tOpen[0]) and float(yLow[0]) < float(yLowerBand[0])):
