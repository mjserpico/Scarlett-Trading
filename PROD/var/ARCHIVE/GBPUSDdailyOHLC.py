...
logger.debug('Starting GBPUSDDailyOHLC')
...
def reply_handler(msg):
    ...
    if float(test) != -1:
            ...
            cur = cnx.cursor()
            cur.execute("""Insert Into GBPUSD (Date, Open, High, Low, Close) values(%s,%s,%s,%s,%s)""",(time.strftime("%m/%d/%Y"),float(test),float(test2),float(test3),float(test4)))
            cnx.commit()
...
logger.debug('Finished GBPUSD Daily OHLC')