...
# test test test
logging.basicConfig(filename='RelativeStrengthBull_945' + str(datetime.date.today()) + '.txt', level=logging.DEBUG, format='%(asctime)s - %(levelname)s - %(message)s')
logger = logging.getLogger()
logger.setLevel(logging.DEBUG)

formatter = logging.Formatter('%(asctime)s - %(levelname)s - %(message)s')

fh = logging.FileHandler('RelativeStrengthBull_945' + str(datetime.date.today()) + '.txt')
...
for row in csv_f:
    ...
    Price = float(truncate((float(gap_close) + (float(ATRentrymultiple[0])*float(ATRVal[0]))),2))
    ...
    logger.debug('15 min high is %s', gap_close)
    logger.debug('15 min high is %s', type(gap_close))
#    if(gap_close > Price):
#        gap_flag = 1
    ...
    if (float(EntryPend[0]) == 0 and float(gap_close) > float(MovAvg[0]) and gap_flag == 0): #
        ...
        CONTRACT = create_contract(Ticker[0], 'STK', 'SMART', 'ISLAND', 'USD')
        logger.debug('Contract Created')
        print(CONTRACT)
