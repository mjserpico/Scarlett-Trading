...
logging.basicConfig(filename='IntradayMgmt_short' + str(datetime.date.today()) + '.txt', level=logging.DEBUG, format='%(asctime)s - %(levelname)s - %(message)s')
logger = logging.getLogger()
logger.setLevel(logging.DEBUG)

formatter = logging.Formatter('%(asctime)s - %(levelname)s - %(message)s')

fh = logging.FileHandler('IntradayMgmt_short' + str(datetime.date.today()) + '.txt')
...
global contickerval
contickerval = ""
logger.debug('contickerval is %s', contickerval)
...
for row in csv_f:
    ...
    logger.debug('At the start of each loop, make sure inPosAPI reset to 0')
    inPosAPI = 0
    logger.debug('inPosAPI flag is %s', inPosAPI)
...
#    time.sleep(0.5)
    ...
    for (Exchange) in cur:
        ...
        logger.debug('Exchnange to route is %s', Exchangeroute[0])
        logger.debug('Exchnange to route type is %s', type(Exchangeroute[0]))
    ...
    logger.debug('CREATE STOP LOSS AND PROFIT TARGET CHECK IS NEXT')
...
###Create Stop Loss order & Profit Target Order after Order has been filled via API
    if (int(curStopLossFlag[0]) == 0 and int(EntryPend[0]) == 1 and inPosAPI == 1):#
        ...
        #StopLoss
        CONTRACT = create_contract(Ticker[0], 'STK', 'SMART', Exchangeroute[0], 'USD')
...
##Profit Target
        ...
        cur.execute(query)
        cnx.commit()
    logger.debug('PAST CREATE STOP LOSS AND PROFIT TARGET CHECK')
    logger.debug('EntryPending is %s', EntryPend[0])
    logger.debug('StopLossFlag is %s', curStopLossFlag[0])
    logger.debug('Order profit is %s', Order_profit)
    logger.debug('Stop Order is %s', Order_stop)
    ...
    logger.debug('Order profit is %s', type(Order_profit))
    logger.debug('Stop Order is %s', type(Order_stop))
    #logger.debug('conTicker %s', conTicker)  #### IS ALWAY THE LAST VALUE IN REPLY HANDLER CHAIN....NEED TO GLOBALIZE CONTTICKER IN REPLY HANDLER WHEN TICKER == conticker
    logger.debug('contickerval for compare %s', contickerval)
    logger.debug('contickerval for compare %s', type(contickerval))
    logger.debug('CLEAR ORDERS NEXT BC POSITON is 0')
#   CLEAR TABLE and ORders because Stop Loss Hit or Profit target was hit --No Position from API        
    if (int(curStopLossFlag[0]) == 1 and int(EntryPend[0]) == 0 and inPosAPI == 0 and int(Order_stop[0]) > 0 and contickerval == Ticker[0]):
        logger.debug('TRAIL Stop loss hit')
...
#Cancel Profit Target     
        ...
        logger.debug('Profit ID is %s', profittrade[0])
        logger.debug('Profit ID is %s', type(profittrade[0]))
...
#        cur.execute(query)
#        cnx.commit()
    logger.debug('BEYOND clear check')
    logger.debug('EntryPend is %s', int(EntryPend[0]))
    logger.debug('Position is %s', int(Positionval[0]))
    logger.debug('stopflag is %s', int(curStopLossFlag[0]))
    logger.debug('profitflag is %s', int(curprofitFlag[0]))
    logger.debug('inPosAPI is %s', inPosAPI)
    #logger.debug('conTicker %s', conTicker)
    logger.debug('Ticker %s', Ticker[0])
    logger.debug('contickerval for compare %s', type(contickerval))
       #### IS ALWAY THE LAST VALUE IN REPLY HANDLER CHAIN....NEED TO GLOBALIZE CONTTICKER IN REPLY HANDLER WHEN TICKER == conticker
    logger.debug('contickerval for compare %s', contickerval)
    logger.debug('Profit Target clear check is next')
    
        
    if (int(curprofitFlag[0]) == 1 and int(EntryPend[0]) == 0 and inPosAPI == 0 and int(Order_profit[0]) > 0 and contickerval == Ticker[0]):
        logger.debug('Profit Target order filled')
        query = ("UPDATE hasPosition SET EntryID = " + "0" + " where CCY =\'" + Ticker[0] +"\';")
        logger.debug('Query is %s', query)
        cur.execute(query)
        cnx.commit()
       ...
       #Cancel Stop Loss     
        ...
        logger.debug('Cancel the trade!! %s',int(stoptrade[0]))
        conn.cancelOrder(int(stoptrade[0]))
        logger.debug('Thy will be done')
...
logger.debug('Disconnecting from IBController')
conn.disconnect()
